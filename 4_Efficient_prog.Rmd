...
def read_file_to_lower():
    with open("data/text.txt", "r") as txt:
        ...
        txt_list_low = [re.sub(' ','',row.lower().strip()) for row in txt_list]
    return txt_list_low
  
txt_clean = read_file_to_lower()
total_chars = sum(len(line) for line in txt_clean)
print(time.time() - t)
print(f"RAM : {process.memory_info().rss - r1} bytes")
...
def read_to_lower():
    with open("data/text.txt", "r") as txt:
        for txt_line in txt:
            yield re.sub(' ','',txt_line.lower().strip())

total_chars = sum(len(line) for line in read_to_lower())
print(time.time() - t)
print(f"RAM : {process.memory_info().rss - r1} bytes")
print(f"Total chars : {total_chars}")
```
...
print(f"Dense : {X.nbytes / 1e6:.1f} Mo") 

sparse_X = csr_matrix(X)
sparse_nbytes = sparse_X.data.nbytes + sparse_X.indices.nbytes + sparse_X.indptr.nbytes
print(f"Sparse : {sparse_nbytes / 1e6:.4f} Mo")
```
...
```{python, echo=TRUE, error=TRUE, collapse=TRUE}
import time
import random

N = 500000

def make_list(zero_prop):
    return [0 if i < zero_prop * N else random.randint(1, 100) for i in range(N)]
...
### 1% errors
```{python, echo=TRUE, collapse=TRUE}
data = make_list(0.01)
t = time.time()
res = [div_if(x) for x in data]
print(f"if/else:    {time.time()-t:.4f}s")

t = time.time()
res = [div_try(x) for x in data]
print(f"try/except: {time.time()-t:.4f}s")
```
...
make_list <- function(zero_prop) {
  c(rep(0, zero_prop * N), sample(1:100, (1 - zero_prop) * N, replace = TRUE))
}
...
t <- Sys.time()
res <- lapply(data, div_if)
cat("if/else:   ", format(Sys.time() - t))


t <- Sys.time()
res <- lapply(data, div_try)
cat("if/else:   ", format(Sys.time() - t))
